...
function makeApiRequestClients($endpoint, $method = 'GET', $data = null, $params = []) {
    global $API_CONFIG;
...
function fetchClientsFromBase($tableId) {
    ...
    try {
        $siteFilter = 'العملاء'; // لو القيمة في الجدول فعلاً "كتلوجات"، سيبها كده. لو "كتالوجات"، غيّرها.
        $results = [];
        $page = 1;
        $pageSize = 100; // جلب 100 سجل في كل صفحة

        do {
            $response = makeApiRequestClients("rows/table/{$tableId}/", 'GET', null, [
                'page' => $page,
                'size' => $pageSize,
