...
:root {
    ...
    --header-height: 70px;
    --transition-expand: all 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94);
    --transition-collapse: all 1.2s cubic-bezier(0.23, 1, 0.32, 1);
    ...
    --transition-ultra-smooth: all 0.8s cubic-bezier(0.19, 1, 0.22, 1);
...
.system-sidebar.expanding {
    transition: var(--transition-expand);
}

.system-sidebar.expanded {
    width: var(--sidebar-expanded);
    box-shadow: 0 8px 40px rgba(151, 126, 43, 0.15);
...
.system-sidebar.expanded .sidebar-header {
    background: rgba(151, 126, 43, 0.06);
    border-bottom-color: rgba(151, 126, 43, 0.2);
    padding: 16px 0;
}
...
.user-avatar {
    ...
    font-size: 1rem;
    transition: var(--transition-bounce);
    box-shadow: 0 2px 8px rgba(151, 126, 43, 0.2);
    flex-shrink: 0;
}

.system-sidebar.expanded .user-avatar {
    transform: scale(1.05);
    box-shadow: 0 4px 16px rgba(151, 126, 43, 0.3);
    margin-left: 12px;
}
...
.system-sidebar.expanded .user-info {
    opacity: 1;
...
.nav-section:nth-child(1) .nav-section-title {
    transition-delay: 0.2s;
}

.nav-section:nth-child(2) .nav-section-title {
...
/* التدرج للقسم الأول - القائمة الرئيسية */
.nav-section:nth-child(1) .nav-item:nth-child(1) .nav-link { transition-delay: 0.25s; }
.nav-section:nth-child(1) .nav-item:nth-child(2) .nav-link { transition-delay: 0.3s; }
.nav-section:nth-child(1) .nav-item:nth-child(3) .nav-link { transition-delay: 0.35s; }
.nav-section:nth-child(1) .nav-item:nth-child(4) .nav-link { transition-delay: 0.4s; }
.nav-section:nth-child(1) .nav-item:nth-child(5) .nav-link { transition-delay: 0.45s; }
.nav-section:nth-child(1) .nav-item:nth-child(6) .nav-link { transition-delay: 0.5s; }
.nav-section:nth-child(1) .nav-item:nth-child(7) .nav-link { transition-delay: 0.55s; }

/* التدرج للقسم الثاني - الإعدادات */
.nav-section:nth-child(2) .nav-item:nth-child(1) .nav-link { transition-delay: 0.65s; }
.nav-section:nth-child(2) .nav-item:nth-child(2) .nav-link { transition-delay: 0.7s; }
.nav-section:nth-child(2) .nav-item:nth-child(3) .nav-link { transition-delay: 0.75s; }
...
.nav-link::before {
    ...
    width: 0;
    height: 60%;
    background: linear-gradient(90deg, transparent, rgba(151, 126, 43, 0.1));
...
.nav-link:hover {
    ...
    box-shadow: 0 4px 12px rgba(151, 126, 43, 0.15);
}
...
.nav-link.active {
    background: var(--gold);
    color: white;
    box-shadow: 0 4px 16px rgba(151, 126, 43, 0.4);
...
.system-sidebar.expanded .language-section {
...
.language-btn::before {
    ...
    width: 100%;
    height: 100%;
    background: linear-gradient(45deg, transparent, rgba(151, 126, 43, 0.1), transparent);
...
.language-btn:hover {
    ...
    box-shadow: 0 4px 12px rgba(151, 126, 43, 0.2);
}
                    ...
                    <ul class="nav-menu">
                        ...
                        <?php if (!empty($systemMenuData)): ?>
                            ...
                            foreach ($menuItems as $menuItem): 
                                // تحديد الأيقونة من قاعدة البيانات أو استخدام الدائرة كافتراضي
                                $iconClass = !empty($menuItem['ايقونة']) ? $menuItem['ايقونة'] : 'fas fa-circle';
                            ?>
                                <li class="nav-item">
                                    <a href="<?php echo !empty($menuItem['الرابط']) ? $menuItem['الرابط'] : '#'; ?>" 
                                       ...
                                       data-page-en="<?php echo sanitizeSystemData($menuItem['page'] ?? $menuItem['الصفحة'] ?? ''); ?>">
                                        <div class="nav-icon"><i class="<?php echo $iconClass; ?>"></i></div>
                    ...
                    ?>
                        <li class="mobile-nav-item">
                            <a href="<?php echo !empty($menuItem['الرابط']) ? $menuItem['الرابط'] : '#'; ?>" 
                               ...
                               data-page-ar="<?php echo sanitizeSystemData($menuItem['الصفحة'] ?? ''); ?>"
                               data-page-en="<?php echo sanitizeSystemData($menuItem['page'] ?? $menuItem['الصفحة'] ?? ''); ?>">
                                <div class="mobile-nav-icon"><i class="<?php echo $iconClass; ?>"></i></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // دالة لإخفاء/إظهار العناصر مع الحركة المحسنة والتدرج العمودي
    function toggleSidebarElements(isExpanded) {
        ...
        if (isExpanded) {
            // إضافة class للتوسيع السريع
            sidebar.classList.add('expanding');
            
            // إظهار معلومات المستخدم أولاً
            if (userInfo) {
                setTimeout(() => {
                    userInfo.style.opacity = '1';
                    userInfo.style.transform = 'translateX(0) scale(1)';
                }, 100);
            }
            
            // إظهار عناوين الأقسام
            ...
            // إظهار نص اللغة أخيراً
            if (languageText) {
                setTimeout(() => {
                    languageText.style.opacity = '1';
                    languageText.style.transform = 'translateX(0) scale(1)';
                }, 800);
            }
        } else {
            // إزالة class التوسيع
            ...
            // إخفاء العناصر بشكل سريع وناعم
            if (userInfo) {
                userInfo.style.opacity = '0';
                userInfo.style.transform = 'translateX(15px) scale(0.9)';
            }
            
            sectionTitles.forEach(title => {
                if (title) {
                    title.style.opacity = '0';
                    title.style.transform = 'translateX(20px) scale(0.8)';
                    setTimeout(() => {
            ...
            navTexts.forEach(text => {
                if (text) {
                    ...
                    text.style.transform = 'translateX(25px) scale(0.9)';
                }
            });
            
            if (languageText) {
                languageText.style.opacity = '0';
                languageText.style.transform = 'translateX(25px) scale(0.9)';
            }
        }
    }
