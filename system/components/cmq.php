...
// ترتيب البيانات
usort($filteredQuotes, function($a, $b) use ($FIELDS) {
    $dateA = new DateTime($a[$FIELDS['quotes']['date']] ?? '1970-01-01');
    $dateB = new DateTime($b[$FIELDS['quotes']['date']] ?? '1970-01-01');
...
<head>
    ...
    <style type="text/tailwindcss">
        ...
        @layer components {
            ...
            .table-row {
                @apply transition-all duration-300 hover:bg-light-gray hover:scale-[1.01] hover:shadow-sm;
            }
            ...
            .modal-content {
                @apply bg-white rounded-xl shadow-xl w-11/12 max-w-2xl max-h-4/5 overflow-hidden;
            }
...
<body>
    ...
    <div class="max-w-7xl mx-auto p-6 min-h-screen">
        <!-- Header -->
        <div class="card mb-6 flex justify-between items-center flex-wrap gap-6">
            <h1 class="text-2xl font-bold text-dark-gray flex items-center gap-3">
                <i class="fas fa-file-invoice-dollar text-gold text-xl"></i>
                إدارة عروض الأسعار
            </h1>
            <div class="flex gap-6 items-center">
                <button onclick="clearAllFilters()" class="btn-gray">
                    <i class="fas fa-eraser"></i>
                    مسح الفلاتر
                </button>
                <button onclick="createNewQuote()" class="btn-gold">
                ...
                <table class="w-full border-collapse text-sm">
                    <thead class="sticky top-0 z-10">
                        <tr>
                            ...
                            <th data-column="price" class="table-header">
                                قيمة العرض
                                <i class="fas fa-sort opacity-30 mr-2 transition-all duration-300"></i>
                            </th>
                    ...
                    <tbody id="quotesTableBody">
                        <?php if (empty($filteredQuotes)): ?>
                            <tr>
                                <td colspan="7" class="text-center py-20 text-medium-gray">
    ...
    <div id="quoteModal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-50">
        <div class="w-full h-full bg-white relative">
            <div class="p-6 border-b border-border bg-gradient-to-r from-light-gray to-gray-50 flex justify-between items-center">
                <div id="modalTitle" class="text-xl font-bold text-dark-gray">عرض السعر</div>
                <button onclick="closeModal()" class="text-2xl cursor-pointer text-medium-gray p-2 rounded-md transition-all duration-300 hover:bg-gray-100 hover:text-dark-gray hover:scale-110">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <iframe id="modalIframe" class="w-full border-0" style="height: calc(100% - 88px);" src="about:blank"></iframe>
        </div>
    </div>
