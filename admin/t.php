...
<body class="gradient-bg font-cairo min-h-screen">
    <!-- الهيدر -->
    <header class="header-gradient shadow-lg border-b border-gray-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center">
                    <img src="https://alfagolden.com/iconalfa.png" alt="شعار" class="w-10 h-10 ml-4">
                    <div>
                        <h1 class="text-xl font-semibold text-white">إدارة المنتجات</h1>
                    </div>
                </div>
                <nav class="hidden md:flex items-center">
                    <button id="showCategoriesBtn" class="text-gray-300 hover:text-white font-medium transition-colors duration-200 p-2 rounded-lg hover:bg-gray-600">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div id="categoriesView" class="view-container">
            <div class="card-gradient rounded-xl shadow-sm border border-gray-100 mb-6">
                <!-- شريط التنقل والتحكم -->
                <div class="px-5 py-4 border-b border-gray-100">
                    <div id="breadcrumb" class="flex items-center text-sm mb-3">
                        <button id="homeBreadcrumb" class="text-gray-600 hover:text-gold transition-colors font-medium breadcrumb-item">
                            <i class="fas fa-layer-group mr-1"></i>الأقسام
                        </button>
                        <span id="currentCategory" class="mr-2"></span>
                    </div>
                    <div class="flex items-center justify-between">
                        <h2 class="text-xl font-semibold text-gray-800">الأقسام</h2>
                        <div class="flex items-center space-x-reverse space-x-2">
                            <!-- أزرار تبديل العرض -->
                            <div class="flex bg-gray-100 rounded-md p-0.5">
                                <button id="gridViewBtn" class="view-toggle active px-2.5 py-1.5 rounded text-sm font-medium">
                                    <i class="fas fa-th"></i>
                                </button>
                                <button id="listViewBtn" class="view-toggle px-2.5 py-1.5 rounded text-sm font-medium text-gray-600">
                                    <i class="fas fa-list"></i>
                                </button>
                            </div>
                            <button id="addCategoryBtn" class="gold-gradient text-white px-4 py-2 rounded-md font-medium transition-all duration-200 hover:shadow-md text-sm">
        ...
        <div id="productsView" class="view-container hidden">
            <div class="card-gradient rounded-xl shadow-sm border border-gray-100 mb-6">
                <!-- شريط التنقل والتحكم -->
                <div class="px-5 py-4 border-b border-gray-100">
                    <div id="breadcrumbProducts" class="flex items-center text-sm mb-3">
                        <button class="text-gray-600 hover:text-gold transition-colors font-medium breadcrumb-item" onclick="showView('categories')">
                            <i class="fas fa-layer-group mr-1"></i>الأقسام
                        </button>
                        <i class="fas fa-chevron-left mx-2 text-gold"></i>
                        <span id="currentCategoryBreadcrumb" class="text-gold font-medium"></span>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-reverse space-x-2">
                            <!-- أزرار تبديل العرض -->
                            <div class="flex bg-gray-100 rounded-md p-0.5">
                                <button id="gridViewBtnProducts" class="view-toggle active px-2.5 py-1.5 rounded text-sm font-medium">
                                    <i class="fas fa-th"></i>
                                </button>
                                <button id="listViewBtnProducts" class="view-toggle px-2.5 py-1.5 rounded text-sm font-medium text-gray-600">
                                    <i class="fas fa-list"></i>
                                </button>
                            </div>
                            <button id="addProductBtn" class="gold-gradient text-white px-4 py-2 rounded-md font-medium transition-all duration-200 hover:shadow-md text-sm">
                                <i class="fas fa-plus mr-1"></i>جديد
                            </button>
                        </div>
                    </div>
    ...
    <div id="categoryModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
        <div class="card-gradient rounded-xl w-full max-w-md shadow-2xl border border-gray-200">
            <div class="p-6">
                <h3 id="categoryModalTitle" class="text-xl font-semibold mb-6 text-gray-800">قسم جديد</h3>
                <form id="categoryForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="categoryName" name="name" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold focus:border-transparent transition-all" placeholder="أدخل اسم القسم..." required>
                    </div>
                    
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">صورة القسم</label>
                        <input type="file" id="categoryImage" name="image" accept="image/*" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold focus:border-transparent transition-all">
                    ...
                    <div class="flex justify-end space-x-reverse space-x-3">
                        <button type="button" id="closeCategoryModal" class="px-6 py-2.5 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-all duration-200">إلغاء</button>
                        <button type="submit" class="gold-gradient text-white px-6 py-2.5 rounded-lg transition-all duration-200 flex items-center hover:shadow-lg">
    ...
    <div id="productModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
        <div class="card-gradient rounded-xl w-full max-w-md shadow-2xl border border-gray-200">
            <div class="p-6">
                <h3 id="productModalTitle" class="text-xl font-semibold mb-6 text-gray-800">منتج جديد</h3>
                <form id="productForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="productName" name="name" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold focus:border-transparent transition-all" placeholder="أدخل اسم المنتج..." required>
                    </div>
                    
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">صورة المنتج</label>
                        <input type="file" id="productImage" name="image" accept="image/*" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gold focus:border-transparent transition-all">
                    ...
                    <div class="flex justify-end space-x-reverse space-x-3">
                        <button type="button" id="closeProductModal" class="px-6 py-2.5 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-all duration-200">إلغاء</button>
                        <button type="submit" class="gold-gradient text-white px-6 py-2.5 rounded-lg transition-all duration-200 flex items-center hover:shadow-lg">
                            <span class="button-text">حفظ</span>
                            <span class="spinner hidden mr-2"></span>
                        </button>
    ...
    <script>
        ...
        let categoriesViewMode = 'grid';
        let productsViewMode = 'grid';
        ...
        const gridViewBtnProducts = document.getElementById('gridViewBtnProducts');
        const listViewBtnProducts = document.getElementById('listViewBtnProducts');
        ...
        // دوال تبديل العرض
        function toggleViewMode(container, mode, viewType) {
            if (viewType === 'categories') {
                categoriesViewMode = mode;
                gridViewBtn.classList.toggle('active', mode === 'grid');
                listViewBtn.classList.toggle('active', mode === 'list');
                gridViewBtn.classList.toggle('text-gray-600', mode !== 'grid');
                listViewBtn.classList.toggle('text-gray-600', mode !== 'list');
            } else {
                productsViewMode = mode;
                gridViewBtnProducts.classList.toggle('active', mode === 'grid');
                listViewBtnProducts.classList.toggle('active', mode === 'list');
                gridViewBtnProducts.classList.toggle('text-gray-600', mode !== 'grid');
                listViewBtnProducts.classList.toggle('text-gray-600', mode !== 'list');
            }
            
            container.classList.toggle('list-view', mode === 'list');
            
            if (mode === 'list') {
                container.querySelector('.grid').classList.remove('grid', 'grid-cols-2', 'md:grid-cols-3', 'lg:grid-cols-4', 'xl:grid-cols-5', 'gap-6');
                container.querySelector('.grid').classList.add('space-y-3');
            } else {
                container.querySelector('.space-y-3')?.classList.remove('space-y-3');
                container.querySelector('div').classList.add('grid', 'grid-cols-2', 'md:grid-cols-3', 'lg:grid-cols-4', 'xl:grid-cols-5', 'gap-6');
            }
        }
                ...
                if (data.list && Array.isArray(data.list) && data.list.length > 0) {
                    ...
                    categoriesGrid.innerHTML = `
                        <div class="col-span-full">
                            <div class="text-center py-16 card-gradient rounded-lg border-2 border-dashed border-gray-200">
                                ...
                                <button onclick="document.getElementById('addCategoryBtn').click()" class="gold-gradient text-white px-6 py-2.5 rounded-lg">
                    ...
                    productsGrid.innerHTML = `
                        <div class="col-span-full">
                            <div class="text-center py-16 card-gradient rounded-lg border-2 border-dashed border-gray-200">
                                ...
                                <button onclick="document.getElementById('addProductBtn').click()" class="gold-gradient text-white px-6 py-2.5 rounded-lg">
        ...
        function createCategoryCard(category) {
            const isListView = categoriesViewMode === 'list';
            const card = document.createElement('div');
            
            if (isListView) {
                card.className = 'list-item cursor-pointer';
                card.innerHTML = `
                    <div class="item-icon">
                        <i class="fas fa-folder text-gray-500 text-lg"></i>
                    </div>
                    <div class="flex-1">
                        <h3 class="font-medium text-gray-800 text-sm">${category.الاسم}</h3>
                    </div>
                    <div class="flex space-x-reverse space-x-1">
                        <button onclick="event.stopPropagation(); editCategory(${category.Id}, '${category.الاسم.replace(/'/g, "\\'")}', '${category.الصورة || ''}')" class="w-7 h-7 bg-gold hover:bg-gold-dark text-white rounded-md flex items-center justify-center transition-colors">
                            <i class="fas fa-edit text-xs"></i>
                        </button>
                        <button onclick="event.stopPropagation(); deleteCategory(${category.Id})" class="w-7 h-7 bg-gray-500 hover:bg-gray-600 text-white rounded-md flex items-center justify-center transition-colors">
                            <i class="fas fa-trash text-xs"></i>
                        </button>
                    </div>
                    ${category.الصورة ? `
                        <div class="image-tooltip">
                            <img src="${category.الصورة}" alt="${category.الاسم}">
                        </div>
                    ` : ''}
                `;
            } else {
                card.className = 'card-gradient rounded-xl p-4 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer card-hover border border-gray-100';
                card.innerHTML = `
                    <div class="relative">
                        ...
                        <div class="absolute top-2 left-2 flex space-x-reverse space-x-1">
                            ...
                            <button onclick="event.stopPropagation(); deleteCategory(${category.Id})" class="w-7 h-7 bg-gray-500 hover:bg-gray-600 text-white rounded-full flex items-center justify-center transition-colors">
                                <i class="fas fa-trash text-xs"></i>
                            </button>
                        </div>
                    </div>
        ...
        function createProductCard(product) {
            const isListView = productsViewMode === 'list';
            const card = document.createElement('div');
            ...
            if (isListView) {
                ...
                card.innerHTML = `
                    ...
                    <div class="flex-1">
                        <h3 class="font-medium text-gray-800 text-sm">${product.الاسم}</h3>
                    </div>
                    <div class="flex space-x-reverse space-x-1">
                        <button onclick="editProduct(${product.Id}, '${product.الاسم.replace(/'/g, "\\'")}', '${product.الصورة || ''}')" class="w-7 h-7 bg-gold hover:bg-gold-dark text-white rounded-md flex items-center justify-center transition-colors">
                            <i class="fas fa-edit text-xs"></i>
                        </button>
                        <button onclick="deleteProduct(${product.Id})" class="w-7 h-7 bg-gray-500 hover:bg-gray-600 text-white rounded-md flex items-center justify-center transition-colors">
                            <i class="fas fa-trash text-xs"></i>
                        </button>
                    </div>
                    ${product.الصورة ? `
                ...
                card.className = 'card-gradient rounded-xl p-4 shadow-sm hover:shadow-lg transition-all duration-300 card-hover border border-gray-100';
                card.innerHTML = `
                    <div class="relative">
                        <div class="w-full h-32 bg-gray-50 rounded-lg mb-3 flex items-center justify-center overflow-hidden">
                            ${product.الصورة ? 
                        ...
                        <div class="absolute top-2 left-2 flex space-x-reverse space-x-1">
                            ...
                            <button onclick="deleteProduct(${product.Id})" class="w-7 h-7 bg-gray-500 hover:bg-gray-600 text-white rounded-full flex items-center justify-center transition-colors">
                                <i class="fas fa-trash text-xs"></i>
                            </button>
                        </div>
                    </div>
        ...
        gridViewBtn.addEventListener('click', () => {
            toggleViewMode(categoriesContainer, 'grid', 'categories');
            loadCategories();
        });

        listViewBtn.addEventListener('click', () => {
            toggleViewMode(categoriesContainer, 'list', 'categories');
            loadCategories();
        });

        gridViewBtnProducts.addEventListener('click', () => {
            toggleViewMode(productsContainer, 'grid', 'products');
            loadProducts(currentCategoryId, currentCategoryName);
        });

        listViewBtnProducts.addEventListener('click', () => {
            toggleViewMode(productsContainer, 'list', 'products');
            loadProducts(currentCategoryId, currentCategoryName);
        });
